...
//	}
	...
	public void visit(DesignatorStatementK1 assign) {
		//niz indeks value or value
		//Code.store(assign.getDesignator().obj);
		if(
				assign.getDesignator().obj.getType().getKind() == Struct.Array) {
			//niz = new int[3]
			Code.store(assign.getDesignator().obj);
			Obj o = assign.getDesignator().obj;
			Obj helper = TabExtended.find("final" + assign.getDesignator().getDesignatorIdent().getIdent());
			if(helper != null) {
				Code.load(o);
				Code.put(Code.arraylength);
				Code.put(Code.newarray);
		...
		}else if(assign.getDesignator().getDesignatorList() instanceof DesignatorListK1){
			//niz[2] = 1;
			Obj helper = TabExtended.find("final" + assign.getDesignator().getDesignatorIdent().getIdent());
			if(helper != null) {// addr indeks
				Code.put(Code.dup2);
				Code.put(Code.pop);		//niz indeks value indeks
				Code.load(helper);
				okreniDvaBroja(); 		//niz indeks value helper indeks
				Code.put(Code.dup2);	//niz indeks value helper indeks helper indeks
				Code.put(Code.aload);
				Code.loadConst(1);
				Code.put(Code.add);		//niz indeks value helper indeks helper[indeks]+1
				Code.put(Code.dup);		//niz indeks value helper indeks helper[indeks]+1 helper[indeks]+1
				Code.loadConst(2);
				int zakrpa = Code.pc;
				Code.putFalseJump(Code.ge, 0);
				//docice ovde ako je prekoraceno
				Code.put(Code.trap);
				Code.put(1);
				
				Code.put2(zakrpa + 1, Code.pc - zakrpa);
				////niz indeks value helper indeks helper[indeks]+1
				Code.put(Code.astore);
				//niz indeks value
				Code.store(assign.getDesignator().obj);
				//Code.load(helper);
				//Code.load(assign.getDesignator().getDesignatorList().get)
			}
		}else {
			Code.store(assign.getDesignator().obj);
		}
